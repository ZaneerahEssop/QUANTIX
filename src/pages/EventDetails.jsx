...
const EventTheme = ({ theme, onUpdate, isEditing = true }) => {
  ...
  const removeColor = (index) => {
    onUpdate({ ...theme, colors: (theme.colors || []).filter((_, i) => i !== index) });
  };

  return (
    <div className="theme-section">
      <div className="section-header"><h2>Event Theme</h2></div>
      {isEditing ? (
        <div className="theme-editor">
          ...
          <div className="form-group">
            ...
            <div className="color-palette">
              {theme?.colors && theme.colors.map((color, index) => (
                <div key={index} className="color-item">
                  ...
                  />
                  <button onClick={() => removeColor(index)} className="remove-color">
                    <FaTimes />
                  </button>
                </div>
              ))}
...
const EventDetails = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (eventData) {
          ...
          // Fetch vendor relationships
          const { data: eventVendors, error: vendorsError } = await supabase
            .from('event_vendors')
            .select('vendor_id')
            .eq('event_id', eventId);
            
          if (!vendorsError && eventVendors) {
            setSelectedVendors(eventVendors.map(ev => ev.vendor_id));
          }
        } else {
  ...
  const handleSave = async () => {
    try {
      ...
      if (selectedVendors.length > 0) {
        ...
        if (vendorError) console.error("Error saving vendor relationships:", vendorError);
      }
  ...
  if (!event) return <div className="error"><h2>Event Not Found</h2><p>The requested event could not be found or you don't have permission to view it.</p><button onClick={() => navigate('/dashboard')}>Back to Dashboard</button></div>;
    ...
    <div className="event-details">
      ...
      <div className="event-info-boxes">
        <div className="info-box date-box">
          ...
          <p>{isEditing ? (<input type="date" name="date" value={formData.date} onChange={handleInputChange}/>) : formatDisplayDate(formData.date)}</p>
        </div>
        <div className="info-box time-box">
          <h4><FaClock /> Time</h4>
          <p>{isEditing ? (<input type="time" name="time" value={formData.time} onChange={handleInputChange}/>) : formData.time || 'Not specified'}</p>
        </div>
                ...
                {selectedVendors.length > 0 ? (
                  <ul>
                    {selectedVendors.map(vendorId => {
                      ...
                      return vendor ? (
                        <li key={vendorId}>
                          <strong>{vendor.business_name}</strong> - {vendor.service_type}<br />
