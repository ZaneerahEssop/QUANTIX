...
export default function VendorDashboard({ session }) {
  ...
  const handleRequestResponse = async (requestId, status) => {
    ...
    try {
      ...
      setSuccessMessage(`Failed to update request status: ${error.message}`);
      setShowSuccess(true);
    }
  };
  ...
  const getUpcomingEvents = (events) => {
    console.log("getUpcomingEvents called with:", events);
    if (!events || !Array.isArray(events)) {
      console.log("No events or invalid events array");
      return [];
    }
    ...
    const filteredEvents = [...events].filter((event) => {
      ...
      if (!event.eventDate) {
        console.log("Event has no date, skipping:", event);
        return false;
      }
      ...
      try {
        ...
        // If the event has a time, set it
        if (event.eventTime) {
          console.log("Event has time:", event.eventTime);
          const [hours, minutes] = event.eventTime.split(":").map(Number);
          if (!isNaN(hours) && !isNaN(minutes)) {
            eventDate.setHours(hours, minutes, 0, 0);
          } else {
            console.warn(
              "Invalid time format, using default time:",
              event.eventTime
            );
          }
        }
    ...
    const sortedEvents = [...filteredEvents].sort((a, b) => {
      try {
        ...
        if (b.eventTime) {
          const [hoursB, minutesB] = b.eventTime.split(":").map(Number);
        ...
        console.log("Comparing:", {
          a: { name: a.eventName, date: dateA.toISOString() },
          b: { name: b.eventName, date: dateB.toISOString() },
          result: comparison,
        });

        return comparison;
      } catch (error) {
        console.error("Error comparing dates:", error, { a, b });
        return 0;
      }
    });
