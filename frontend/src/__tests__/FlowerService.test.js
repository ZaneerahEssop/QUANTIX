...
    let utils;
    // FIX: We render outside act, but must wrap the asynchronous functions 
    // being called inside useEffect on mount (fetchServiceData)
    // We will rely on waitForLoadingComplete to handle the async updates.
...
      await renderComponent();
      ...
      // The empty state text is rendered inside ReactMarkdown components
      const markdownElements = screen.getAllByTestId('react-markdown');
      expect(markdownElements.length).toBeGreaterThan(0);
      expect(markdownElements[0]).toHaveTextContent('No description provided.');
      expect(markdownElements[1]).toHaveTextContent('No specialties listed.');
      expect(markdownElements[2]).toHaveTextContent('No sourcing information provided.');
      expect(markdownElements[3]).toHaveTextContent('No event types specified.');
      expect(markdownElements[4]).toHaveTextContent('No base rate specified.');
    });
...
      // FIX: Wrap the asynchronous state update that happens after clicking 'Cancel'
      await act(async () => {
...
      
      // FIX: The change event handler includes a state update, so it needs act
      await act(async () => {
...
      
      // FIX: Wrap the submission which triggers the asynchronous save/refetch
      await act(async () => {
...
      const form = document.getElementById('service-form');
      
      // FIX: Wrap the submission which triggers the asynchronous save/error handling
      await act(async () => {
        fireEvent.submit(form);
      });
...
      
      // FIX: File change triggers an async upload and state update, so it needs act
      await act(async () => {
...
      // FIX: Click handler triggers async removal and state update, so it needs act
      await act(async () => {
...
      const deleteButtons = screen.getAllByText('×');
      // FIX: Click handler still needs act even if logic is synchronous
      await act(async () => {
        fireEvent.click(deleteButtons[0]);
      });
...
      // FIX: Wrap the submission which triggers the asynchronous save/toast display
      await act(async () => {
        fireEvent.submit(form);
      });
...
      // FIX: The click event handler needs act wrapping
      await act(async () => {
...
      // FIX: Advancing timers causes a state update, so it needs act
      act(() => {
...
      // FIX: File change triggers validation/state update, so it needs act
      await act(async () => {
...
      const deleteButtons = screen.getAllByText('×');
      // FIX: Click handler triggers async remove and state update, so it needs act
      await act(async () => {
        fireEvent.click(deleteButtons[0]);
      });
...
      const form = document.getElementById('service-form');
      
      // FIX: Wrap the submission which triggers the asynchronous save
      await act(async () => {
        fireEvent.submit(form);
      });
...
      // FIX: File change triggers an async function, so it needs act
      await act(async () => {
        fireEvent.change(fileInput, { target: { files: [file] } });
      });
